...
import { type SyncObservable, mapSyncObservable } from '../misc/observable'
...
export function createResolvedConfigurationObservable(
    input: SyncObservable<ConfigurationInput>
): Promise<SyncObservable<ResolvedConfiguration>> {
    return mapSyncObservable(input, resolveConfiguration)
}
