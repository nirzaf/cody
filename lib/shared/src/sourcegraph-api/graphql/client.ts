...
export class SourcegraphGraphQLAPIClient {
     ...
     */
    private config: SyncObservable<Pick<ResolvedConfiguration, 'configuration' | 'auth'>> | null = null

    private isAgentTesting = process.env.CODY_SHIM_TESTING === 'true'

    public setResolvedConfigurationObservable(
        config: SyncObservable<Pick<ResolvedConfiguration, 'configuration' | 'auth'>>
    ): void {
        this.config = config
    }
        ...
        return this.fetchSourcegraphAPI<APIResponse<ContextSearchResponse>>(
            ...
            extractDataOrError(response, data =>
                (data.getCodyContext || []).map(item => ({
                    ...
                    path: item.blob.path,
                    uri: URI.parse(
                        `${this.config!.value.auth.serverEndpoint}${item.blob.repository.name}/-/blob/${
                            item.blob.path
    ...
    public async logEvent(event: event, mode: LogEventMode): Promise<LogEventResponse | Error> {
        ...
        if (this.config!.value.configuration.telemetryLevel === 'off') {
            return {}
        }
        /**
         ...
         * the same thing.
         */
        if (isDotCom(this.config!.value.auth.serverEndpoint)) {
    ...
    public async fetchSourcegraphAPI<T>(
        ...
        const headers = new Headers(config.auth.customHeaders as HeadersInit)
...
export class ClientConfigSingleton {
    ...
    public async setAuthStatus(authStatus: AuthStatus, signal?: AbortSignal): Promise<void> {
        // TODO!(sqs)
        this.isSignedIn = authStatus.isLoggedIn
        if (this.isSignedIn) {
